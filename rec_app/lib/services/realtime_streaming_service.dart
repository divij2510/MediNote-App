...
class RealtimeStreamingService extends ChangeNotifier {
  ...
  Future<void> _connectWebSocket() async {
    try {
      ...
      debugPrint('WebSocket connected successfully to: $wsUrl');
    } catch (e) {
  ...
  Future<void> _startAudioRecording() async {
    try {
      ...
      _currentRecordingPath = '${directory.path}/temp_recording_${DateTime.now().millisecondsSinceEpoch}.m4a';
      ...
      try {
        ...
        if (_currentRecordingPath != null && File(_currentRecordingPath!).existsSync()) {
          ...
          if (fileBytes.isNotEmpty) {
            // Stream actual audio data
            _audioController?.add(fileBytes);
          }
        }
      } catch (e) {
  ...
  void _startChunkProcessing() {
    ...
    _chunkTimer = Timer.periodic(const Duration(seconds: 5), (timer) async {
      if (_isStreaming && !_isPaused && !_isProcessingChunk) {
  ...
  Future<void> _processAudioChunk() async {
    if (_isProcessingChunk || _currentSession == null || _currentRecordingPath == null) return;
    ...
    try {
      ...
      if (currentFile.existsSync()) {
        ...
        if (fileBytes.isNotEmpty) {
          ...
          await _sendChunkToServer(fileBytes);
        }
      }
  ...
  Future<void> _sendChunkToServer(Uint8List chunkData) async {
    if (_channel != null && _channel!.closeCode == null) {
      try {
        // Send chunk data to server
        _channel!.sink.add(chunkData);
        debugPrint('Sent chunk $_chunkNumber to server (${chunkData.length} bytes)');
      } catch (e) {
        debugPrint('Error sending chunk to server: $e');
  ...
  Future<void> stopStreaming() async {
    ...
    try {
      ...
      // Process final chunk
      if (_currentRecordingPath != null && File(_currentRecordingPath!).existsSync()) {
        final finalFile = File(_currentRecordingPath!);
